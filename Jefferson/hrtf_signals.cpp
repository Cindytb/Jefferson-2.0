...
//{
//	int i, n, ele_idx, azi_idx, obj_azi_sign, hrtf_idx;
//	float d, dmin;
//
...
//		}
//	}
//	printf("Azimuth position: %f\n", azi_idx * azimuth_inc[ele_idx]);
//
//	/* return hrtf index */
...
//	return *hrtf;
//}
//int convolve_hrtf(float *x, int x_len, int hrtf_idx, float *output, int y_len, float gain)
//{
...
//		output[i] = 0.0;
//	}
//	/*GPU Convolution not working fast enough*/
//	//convolveMe(output, x, x_len, p_hrtf, gain);
//	for (n = 0; n < y_len; n++) {
//		for (k = 0; k < HRTF_LEN; k++) {
//			for (j = 0; j < HRTF_CHN; j++) {
//				/* y_len and HRTF_LEN are n frames, output and hrtf are interleaved
//				* x is mono
//				*/
//				j_hrtf = (swap_chan == false) ? j : (j == 0) ? 1 : 0;
//				output[2 * n + j] += x[n - k] * p_hrtf[2 * k + j_hrtf];
//			}
//			output[2 * n] *= gain;
//			output[2 * n + 1] *= gain;
//		}
